...
private:
    ...
    void resize()
    {
        ...
        /* TODO: capacity의 크기를 2배로 늘리고, 새로운 배열을 생성하세요. */
        size_t newCapacity = capacity * 2;
        int *newData = new int[newCapacity];

        /* TODO: 기존 데이터를 새로운 배열로 복사한 뒤, 기존 배열을 해제하세요. */
        ...
        data = newData;         // 포인터 갱신
        capacity = newCapacity; // capacity 값 갱신
    }
...
public:
    ...
    {
        /* TODO: data는 capacity 크기의 배열을 동적 할당하세요. */
        data = new int[capacity];
    }

    ~MyIntVector()
    {
        /* TODO: 동적 할당된 data를 해제하세요. */
        if (data != nullptr)
        {
            delete[] data;
        }
    ...
    {
        /* TODO: length가 capacity에 도달하면 resize()를 호출하세요. */
        if (length == capacity)
        {
            resize();
        }

        /* TODO: 새로운 요소를 배열 끝에 추가하고 length를 증가시키세요. */
        data[length] = value;
        length++;
    }
    ...
    int pop_back()
    {
        ...
        return data[--length]; // 마지막 요소 제거 (실제 메모리는 유지, 인덱스만 줄임)
    }
